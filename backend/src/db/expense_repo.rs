...
impl Database {
    ...
    pub fn list_expenses_by_patient_paginated(
        &self,
        patient_id: &str,
        page: usize,
        page_size: usize,
    ) -> Result<PaginatedList<DailyExpenseSummary>, AppError> {
        self.with_conn(|conn| {
            let total: usize = conn.query_row(
                "SELECT COUNT(*) FROM daily_expenses WHERE patient_id = ?1",
                [patient_id],
                |row| row.get::<_, i64>(0),
            )? as usize;
            ...
            let expense_rows = stmt.query_map(
                rusqlite::params![patient_id, page_size as i64, offset as i64],
                |row| {
                    Ok(DailyExpense {
                        id: row.get(0)?,
                        patient_id: row.get(1)?,
    ...
    pub fn delete_expense(&self, id: &str) -> Result<(), AppError> {
        self.with_conn(|conn| {
            let exists: Option<String> = conn
                ...
                .optional()?;
            if exists.is_none() {
                return Err(AppError::expense_not_found());
            }
